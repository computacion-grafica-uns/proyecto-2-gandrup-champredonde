    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 CookTorranceSpecular(float3 N, float3 V, float3 L, float3 lightColor)
            {
                ...
                float3 F0 = lerp(float3(0.04, 0.04, 0.04), _MaterialKs.rgb, 1.0);
            ...
            float3 BlinnPhongDiffuse(float3 N, float3 L, float3 lightColor)
            {
                float NdotL = max(dot(N, L), 0.0);
                return _MaterialKd.rgb * lightColor * NdotL;
            }
            ...
            fixed4 fragmentShader(v2f f) : SV_Target
            {
                ...
                if(_DirActiva == 1)
                {
                    ...
                    float3 diff1 = BlinnPhongDiffuse(N, L1, _DirLightColor.rgb);
                ...
                if(_PointActiva == 1)
                {
                    ...
                    float3 diff2 = BlinnPhongDiffuse(N, L2, _PointLightColor.rgb * atten);
                ...
                if(_SpotActiva == 1)
                {
                    ...
                    {
                        float3 diff3 = BlinnPhongDiffuse(N, L3, _SpotLightColor.rgb);
