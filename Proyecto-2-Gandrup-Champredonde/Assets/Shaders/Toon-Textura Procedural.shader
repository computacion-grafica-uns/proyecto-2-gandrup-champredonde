Shader "Practica/Toon-Textura Procedural"
{
    Properties
    {
        // Material
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 GenerateProceduralTexture(float2 uv)
            {
                // Checkerboard procedural texture
                float2 scaledUV = uv * _Tile;
                float checker = fmod(floor(scaledUV.x) + floor(scaledUV.y), 2.0);
                return lerp(float3(1, 1, 1), float3(0.2, 0.2, 0.2), checker); // blanco y gris oscuro
            }
            ...
            fixed4 frag(v2f f) : SV_Target
            {
                ...
                float3 texColor = GenerateProceduralTexture(f.uv).rgb;
