Shader "Practica/Toon"
{
    Properties
    {
        ...
        _MaterialKd ("Diffuse", Color) = (1, 1, 1, 1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f f) : SV_Target
            {
                ...
                float3 diffColor = _MaterialKd.rgb;
                ...
                if (_DirActiva > 0.5)
                {
                    ...
                    color += _DirLightColor.rgb * (toonDiff * diffColor + toonSpec * _MaterialKs.rgb);
                }
                ...
                if (_PointActiva > 0.5)
                {
                    ...
                    float shininess = (1.0 - roughness) * 128.0;
                    float toonSpec = pow(ToonShade(dot(N, H)), shininess);
                    color += _PointLightColor.rgb * atten * (toonDiff * diffColor + toonSpec * _MaterialKs.rgb);
                }
                ...
                if (_SpotActiva > 0.5)
                {
                    ...
                    if (angle < _SpotLightApertura)
                    {
                        ...
                        float shininess = (1.0 - roughness) * 128.0;
                        float toonSpec = pow(ToonShade(dot(N, H)), shininess);
                        color += _SpotLightColor.rgb * (toonDiff * diffColor + toonSpec * _MaterialKs.rgb);
                    }
                }
